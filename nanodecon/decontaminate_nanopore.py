...
def co_occuring_mutations_in_reads(arguments, lower_confirmed_mutation_dict, gene_score_dict, fsa_file, allele_pair_dict):
    ...
    for allele in lower_confirmed_mutation_dict:
        ...
        if num_mutations > 1:
            ...
            print("Mutation names:", mutations)
            for i, row in enumerate(co_occurrence_matrix):
                mutation_name = mutations[i]
...
def validate_mutations(arguments, mutation_position_dict, gene_score_dict, fsa_file):
    ...
    for item in mutation_position_dict:
        ...
        for i in range(len(mutation_position_dict[item][1])):
            if mutation_position_dict[item][1][i] in mutations_found_in_rmlst_genes[item.split('_')[0]]:
